...
function go(page){ state.page=page; state.mobileMenuOpen=false; render(); }
function toggleMobileMenu(){ state.mobileMenuOpen=!state.mobileMenuOpen; render(); }
function logout(){ state.portal=null; state.role=null; state.page=null; state.mobileMenuOpen=false; render(); }
...
function shell(content){
  ...
  const mobileQuick = state.portal==="gestao" ? nav.slice(0,4) : nav.slice(0,5);
  ...
  return `<div class="portal-layout">
    ...
    <main class="main">
      <header class="topbar">
        <div class="mobile-brand">
          <button class="mobile-menu-button" onclick="toggleMobileMenu()" aria-label="Abrir menu">☰</button>
          <div class="logo">O</div>
          <div><strong>ODR</strong><small>${info.label}</small></div>
        </div>
        ...
        <div class="userbox">
          <div class="avatar">${info.name[0]}</div>
          <div class="user-meta"><strong>${info.name}</strong><div style="font-size:10px;color:var(--muted)">${info.label}</div></div>
          <button class="btn btn-light logout-btn" onclick="logout()">Sair</button>
        </div>
      </header>
    ...
    <aside class="mobile-menu-panel">
      <div class="mobile-menu-head">
        <div><strong>${info.label}</strong><small>${info.name}</small></div>
        <button onclick="toggleMobileMenu()">×</button>
      </div>

      <div class="mobile-menu-list">
        ${nav.map(([p,i,l])=>`<button class="${state.page===p?'active':''}" onclick="go('${p}')"><span>${i}</span><strong>${l}</strong></button>`).join("")}
      </div>

      ${state.portal==="gestao" ? `
        <div class="mobile-menu-section">Trocar visão administrativa</div>
        <div class="mobile-role-list">
          ${["gestor","ouvidoria","auditor","admin","direcao"].map(r=>`<button class="${state.role===r?'active':''}" onclick="setRole('${r}')">${ROLE_INFO[r].label}</button>`).join("")}
        </div>
      `:""}
    </aside>`:""}

    <nav class="mobile-nav">
      ${mobileQuick.map(([p,i,l])=>`<button class="${state.page===p?'active':''}" onclick="go('${p}')"><span class="mobile-nav-icon">${i}</span><span>${l}</span></button>`).join("")}
      ${state.portal==="gestao" ? `<button onclick="toggleMobileMenu()"><span class="mobile-nav-icon">☰</span><span>Mais</span></button>`:""}
    </nav>
  </div>`;
}
...
function userCasesTable(rows, detailPage){
  return `<div class="card mobile-protocols"><div class="table-wrap"><table>
...
function render(){
  ...
  if(state.portal==="gestao" && !state.role){
    document.body.className="auth-screen";
    managementLogin();
    return;
  }

  document.body.className="app-screen";
